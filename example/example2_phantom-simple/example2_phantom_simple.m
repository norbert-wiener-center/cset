...
%  Usage:
%  =====
%  Creates CS-ET and WBP reconstructions of the simple membrane phantom
%  used in (Guay et al., 2016).

%% Setup

% Get the simple phantom dataset, add additional subfolders to the
% search path.
setup('phantom-simple');

% Load the simple phantom;
load('psimple.mat');

% If the tilt series doesn't already exist, create it.
if exist('tilt.mat', 'file') ~= 2
    disp('Simple phantom tilt series not found. Creating it now.');
    % Load reconstruction data, to get projection angles.
    load('recdata.mat');
    % Create the tilt series.
    tilt = radon3(psimple, recdata.theta); %#ok<NASGU>
    % Save the tilt series.
    save('data/phantom-simple/tilt.mat', 'tilt');
    % Clean up, even though we'll just load them again in a second.
    clear tilt recdata;
end

% Load the simple phantom projections and additional reconstruction data -
...
projsn = add_gaussian_noise(projs, 600, 420);
...
% Display the image. Compare with example2_phantom_simple.png. If you
